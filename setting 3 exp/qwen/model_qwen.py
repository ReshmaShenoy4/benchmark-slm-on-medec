...
#model = "dmis/meerkat-7b:latest"
#model = "jsk/bio-mistral:latest"
#model = "koesn/llama3-openbiollm-8b:q5_K_M"
#model = "phi3.5"
#model = "phi3:instruct"
#model = "hf.co/oceansweep/MedMobile-Q8_0-GGUF"
#model = "meditron:7b"
#model = "hf.co/MaziyarPanahi/BioMedGPT-LM-7B-GGUF"
#model = "hf.co/bartowski/gemma-2-2b-it-GGUF:gemma-2-2b-it-Q8_0.gguf"
#model = "qwen2.5:3b-instruct"
#model = "llama3.2:3b"
#model =  "hf.co/FreedomIntelligence/Apollo-7B-GGUF"
#model = "hf.co/TheBloke/dragon-mistral-7B-v0-GGUF"


models = [ 
#          ("dmis/meerkat-7b:latest", "MR_Meerkat_st2.txt","MR_Meerkat_print.txt"),
#          ("jsk/bio-mistral:latest", "MR_BioMistral_st2.txt", "MR_BioMistral_print.txt"),
#          ("koesn/llama3-openbiollm-8b:q5_K_M","MR_OpenBio_st2.txt","MR_OpenBio_print.txt"),
#          ("meditron:7b","MR_Meditron_st2.txt","MR_Meditron_print.txt"),
#          ("hf.co/bartowski/Phi-3-medium-128k-instruct-GGUF","MR_Phi3_medium_st2.txt","MR_Phi3_medium_print.txt"),
#          ("hf.co/oceansweep/MedMobile-Q8_0-GGUF","MR_MedMobile_st2.txt","MR_MedMobile_print.txt"),
#          ("hf.co/MaziyarPanahi/BioMedGPT-LM-7B-GGUF","MR_BioMed_st2.txt","MR_BioMed_print.txt"),
           ("qwen2.5:3b-instruct", "model_response.txt", "invalid_lines.txt"),
#          ("phi3.5","MR_Phi3.5_st2.txt", "MR_Phi3.5_print.txt"),
#          ("hf.co/bartowski/gemma-2-2b-it-GGUF:gemma-2-2b-it-Q8_0.gguf", "MR_Gemma_st2.txt","MR_Gemma_print.txt"),           
#          ("llama3.2:3b","MR_Llama_st2.txt","MR_Llama_print.txt"),
#          ("hf.co/FreedomIntelligence/Apollo-7B-GGUF","MR_Apollo_st2.txt","MR_Apollo_print.txt"),
#          ("hf.co/TheBloke/dragon-mistral-7B-v0-GGUF","MR_Dragon_st2.txt","MR_Dragon_print.txt")
#           ("chevalblanc/gpt-4o-mini","MR-GPT-4o.txt","MR-GPT-4o-print.txt"),
#           ("hf.co/bartowski/Phi-3.5-mini-instruct-GGUF","MR_Phi3.5_st2.txt", "MR_Phi3.5_print.txt"),
...
base_prompt = """<|im_start|>system: You are a skilled medical doctor reviewing clinical text for factual or diagnostic errors.
...
For example:
text-id-1 0 -1 NA
text-id-2 1 8 "correction of sentence 8 is ..."
...
{sentences}
\n"""
    full_prompt = base_prompt + "\n" + eg_text + "Please answer the following question:\n" + question + "<|im_end|>"
...
def parse_run_submission_file(filepath,print_file):
    ...
    with open(print_file,"w",encoding="utf-8") as f:    
        for line in lines:
            ...
            if len(line) == 0:
                continue
            if not re.search(r'ms-test-\d+\s[01]\s-?\d+(?:\s.+)?', line, re.IGNORECASE):   
            ...
            for i, token in enumerate(items):
                if token.startswith("ms-test-"):
                    text_id = items[i]
                    if i+1 < len(items) and items[i + 1] in ("0", "1"):
                        error_flag  = items[i + 1]
                        if i+2 < len(items):
                            sentence_id = items[i + 2]
                            if i+3 < len(items):
                                corrected_sentence = ' '.join(items[i+3:]).strip()
