...
models = [ 
("hf.co/bartowski/gemma-2-2b-it-GGUF:gemma-2-2b-it-Q8_0.gguf", "model_response.txt","invalid_line.txt")
]
...
base_prompt = """<bos><start_of_turn> user
...
for model,output_file,print_file in models:
    ...
    with open(output_file,"w",encoding="utf-8") as f:
        ...
        for i,row in test_df.iterrows():
            text_id = str(row["Text ID"])
            inp = fixed_prompts[text_id]
            #result = subprocess.run(["ollama","run",model,"-o","top_p=1","-o","num_predict=200","--stdin"],input=inp,text=True,capture_output=True)

            result = subprocess.run(["ollama","run",model,inp],text=True,capture_output=True)
