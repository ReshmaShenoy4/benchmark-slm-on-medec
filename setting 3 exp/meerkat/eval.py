...
def compute_accuracy(reference_flags,reference_sent_id,predicted_flags,candidate_sent_id):
 # Error Flags Accuracy (missing predictions are counted as false)
    ...
    reference_total_error = 0
    for text_id in reference_flags:
        reference_total_error += reference_flags[text_id] == "1"
        if text_id in predicted_flags and reference_flags[text_id] == predicted_flags[text_id]:
            matching_flags_nb += 1
            if text_id in candidate_sent_id and candidate_sent_id[text_id] == reference_sent_id[text_id] and (str(predicted_flags.get(text_id, "0")).strip() != "0"):
                matching_sentence_nb += 1
                dual_print(f"Matched text_id and its candidate_id is ",text_id, candidate_sent_id[text_id])
            
    flags_accuracy = matching_flags_nb / len(reference_flags)
    sent_accuracy = matching_sentence_nb / reference_total_error
...
class NLGMetrics(object):
    ...
    def compute(self, references, predictions, counters):
        ...
        if 'ROUGE' in self.metrics:
            ...
            composite_score_rouge1 = (rouge1score + counters["model_provided_correct_na"]) / counters["total_texts"]
            composite_score_rouge2 = (rouge2score + counters["model_provided_correct_na"]) / counters["total_texts"]
            composite_score_rougeL = (rougeLscore + counters["model_provided_correct_na"]) / counters["total_texts"]
        ...
        if 'BERTSCORE' in self.metrics:
            ...
            composite_score_bert = (bertscores.sum() + counters["model_provided_correct_na"]) / counters["total_texts"]
        ...
        if 'BLEURT' in self.metrics:
            ...
            composite_score_bleurt = (bleurtscores.sum() + counters["model_provided_correct_na"]) / counters["total_texts"]
        ...
        if aggregate_components > 0:
            ...
            composite_score_agg = (aggregate_subset_scores.sum() + counters["model_provided_correct_na"]) / counters["total_texts"]
...
def get_nlg_eval_data(reference_corrections,reference_flags, candidate_corrections,candidate_flags, remove_nonprint = False):
    references = []
    predictions = []
    ...
    counters = {
        ...
        "total_model_texts": 0,
        "model_provided_na": 0,
        "model_provided_correct_na": 0,
    }
   
    # For each text_id in reference, tex_id in candidate is checked. And if there is, then depending upon its content, it is either appended or not. So, that is in sequence. Resh
    for text_id in reference_corrections:
        ...
        if text_id in candidate_corrections:
            increment_counter(counters, "total_model_texts")
            ...
            if candidate == "NA":
                increment_counter(counters, "model_provided_na")
                
            # matching NA counts as 1
            if reference_correction == "NA" and candidate == "NA":
                increment_counter(counters, "model_provided_correct_na")
                continue
            ...
            # Only when candidate has information it will append, irrespective of error flag - This is changed. Resh
            if reference_flags[text_id] == candidate_flags[text_id]: 
                dual_print(f"\nIn eval.py for sentence comparion for {text_id} ---")
...
# Rouge
references, predictions, counters = get_nlg_eval_data(reference_corrections, reference_flags, candidate_corrections, candidate_flags)
