...
#--------------- MODEL LIST ---------------#
check_point = "dmis-lab/meerkat-7b-v1.0" 
models = [ 

          (check_point, "model_response.txt","invalid_lines.txt"),
]
...
#SYSTEM_CONTENT = (
#       """ You are a skilled medical doctor reviewing clinical text for factual or diagnostic errors.The entire text is either fully correct or has 1 error.
#        TASK:
...
#        -The first four tokens must always be: text_id, error_flag, error_sentence_id, corrected_sentence/NA"""
#)
...
{sentences}
\n"""
    USER_CONTENT = "Please answer the following question:\n" + question 
    prompts[text_id] =  USER_CONTENT 
...
def parse_run_submission_file(filepath,print_file):
    ...
    with open(print_file,"w",encoding="utf-8") as f:    
        for line in lines:
            ...
            pedicted_flags[text_id] = error_flag
...
for model_name,output_file,print_file in models:
    ...
    with open(output_file,"w",encoding="utf-8") as f:
        ...
        for i,row in test_df.iterrows():
            text_id = str(row["Text ID"])
            user_content = fixed_prompts[text_id]
            messages = [
                {"role": "system", "content": SYSTEM_CONTENT},
                {"role": "user",   "content": user_content},
            ]
            encodeds = tokenizer.apply_chat_template(messages, tokenize=True,add_generation_prompt=True,return_tensors="pt").to("cuda")
            model_inputs = encodeds.to("cuda")
            model.to("cuda")
            generated_ids = model.generate(model_inputs, max_new_tokens=1000, do_sample=False, pad_token_id=tokenizer.eos_token_id, eos_token_id = tokenizer.eos_token_id)
            decoded = tokenizer.batch_decode(generated_ids)
            #result = subprocess.run(["ollama","run",model,full_prompt],text=True,capture_output=True)
            model_response = decoded[0].strip()
            match = re.search(r"^[a-z0-9\-]+\s[0-9]+\s\-?[0-9]+\s.+", model_response, flags=re.M)
            model_response = match.group(0) if match else model_response.splitlines()[-1].strip()
